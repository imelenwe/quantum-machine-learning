...
2. **[MaxCut: Three Real Representations](https://imelenwe.github.io/quantum-machine-learning/QAOA/learn/maxcut_example_usecases.html)** — reference, with diagrams. The same coloring game wearing three different costumes: magnets that want to disagree, party guests split from their rivals, and WiFi routers avoiding shared channels.
...
5. **[Classical vs Quantum, Side by Side](https://imelenwe.github.io/quantum-machine-learning/QAOA/learn/classical_vs_quantum.html)** — interactive, real data. Brute force explained on its own, then the quantum circuit explained on its own, then the two lined up — every number pulled straight from the notebook, plus a p-slider showing probability concentrate onto the true answer live.
